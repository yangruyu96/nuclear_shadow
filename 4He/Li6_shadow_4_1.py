...
def state(x):
    y = np.loadtxt('time_{}_Li6_new'.format(x),dtype=complex)
    return y
...
h_ex = np.zeros([64,64],dtype=complex)
...
state_shadow = np.loadtxt('shadow_80_1k_Li6_4_94_new',dtype=complex)

# for i in tqdm(range(1,21)):
#     print(state(i))
#shadow(10,H,1,index = 1)
for j in tqdm(range(81,600,1)):
    for i in range(0,1000):
        state_shadow = (state_shadow + shadow(6,H,1,index = 4)/1000)
    if j % 10 == 0:
        np.savetxt('shadow_{}_1k_Li6_4_94_new'.format(j),state_shadow)
#
# number = 10
# simulator = QasmSimulator()
# circuit = QuantumCircuit(number, number)
