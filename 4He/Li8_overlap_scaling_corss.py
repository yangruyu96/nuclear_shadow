...
def state(i,n):
    state = np.loadtxt('shadow_{}_1k_Li8_{}_94_new'.format(n,i),dtype=complex)  #* 1000
...
for n in range(0,30,1):
    ...
    x_value.append((n) ** (1))
    for bar in (1,1):
        state_overlap = multi_trace([state(4,n*10),state(2,n*10),H,state(3,n*10),state(4,n*10)])
        state_overlap_ideal = multi_trace([state_ideal(4),state_ideal(2),H,state_ideal(3),state_ideal(4)])
        print(state_overlap)
