...
def state(x):
    y = np.loadtxt('time_{}_He6_new'.format(x),dtype=complex)
    return y
...
for j in tqdm(range(0,11,1)):
    for i in range(0,100):
        state_shadow = (state_shadow + shadow(4,H,1,index =25)/100)
    if j % 10 == 0:
        np.savetxt('shadow_{}_1h_He6_25_94_new'.format(j),state_shadow)
