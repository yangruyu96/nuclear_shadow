...
def state(i,n):
    state = np.loadtxt('shadow_{}_1h_Bo7_{}_94_new'.format(n,i),dtype=complex)*100 #+ np.loadtxt('shadow_{}_1_He6'.format(i),dtype=complex) * 10000
    ...
    if i != refe and j != refe:
        ...
        if i != refe:
            ...
            if j != refe:
                ...
                return (multi_trace([state_ideal(refe,n),state_ideal(3,n),state_ideal(refe,n)])/multi_trace([state_ideal(refe,n),state_ideal(3,n)]))
    ...
    if i != refe and j != refe:
        ...
        if i != refe:
            ...
            if j != refe:
                ...
                return (multi_trace([state(refe,n),state(3,n),state(refe,n)])/multi_trace([state(refe,n),state(3,n)]))
...
N_matrix = np.zeros([3,3],dtype=complex)

H_matrix_ideal = np.zeros([3,3],dtype=complex)
N_matrix_ideal = np.zeros([3,3],dtype=complex)
...
x_value = []
y_value = []
for n in tqdm(range(100,400,10)):
