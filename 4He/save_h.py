...
import scipy
#print('li7')
H_pauli = np.loadtxt('Be7_pauli',dtype=complex)
            ...
            for d in range(0, 4):
                for e in range(0,4):
                    for f in range(0,4):
                        for g in range(0,4):
                            print(a,b,c,d,e)
                            H = H + (H_pauli[term] * pauli_index(a)^pauli_index(b)^pauli_index(c) ^ pauli_index(d) ^ pauli_index(e)^pauli_index(f)^pauli_index(g))
                            term = term + 1
